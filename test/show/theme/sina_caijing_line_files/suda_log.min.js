...
(function(){function addLoadEvent(fn){document.addEventListener('DOMContentLoaded',fn,false);}
...
startTime='',readyTime=''
currTime=parseInt(new Date().getTime());var wm='',urlParameter=loc.split('?')[1];if(!!urlParameter&&loc.indexOf('wm=')>0){var pArr=urlParameter.split('&'),len=pArr.length;for(var i=0;i<len;i++){var arg=pArr[i].split('=');if(arg[0]=='wm'&&arg[1]!='#column'){wm=arg[1];break;}}}
...
window.SUDA=window.SUDA||[];SUDA.sudaCount=SUDA.sudaCount||0;SUDA.log=function(){gatherCommon.apply(null,arguments);};SUDA.uaTrack=function(){uaTrack.apply(null,arguments);};SUDA.uaTrackLike=function(){uaTrackLike.apply(null,arguments);};addEvent(doc.body,'click',trackAgent);gatherCommon2();}
addLoadEvent(function(){getUserInfo(function(re){if(re.uid){sudaLogInit(re.uid);}else{sudaLogInit();}},true)});window.checkLogin=function(){if(getCookie('SUBP')){var obj=getSUBPCookie.decode(getCookie('SUBP'));if(typeof obj=='object'){if(obj.status==="0"){var bloon=true;}
...
jsonp({success:function(rs){rs.data=rs.data||{};if(rs.data.islogin&&rs.data.uid){setCookie('sina_ucode',convNum(rs.data.uid),240,'/','.sina.cn');}
window.userInfo=rs.data;if(typeof(fn)=='function'){fn(rs.data);}},error:function(){fn({});},url:url,data:data,timeout:3000})}
...
return decodeURIComponent(out);};var byte2Hex=function(b){var str="0"+b.toString(16);return str.length<=2?str:str.substr(1);};})();